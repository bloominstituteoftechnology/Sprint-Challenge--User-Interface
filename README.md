...
1. If you were to describe semantic HTML to the next cohort of students, what would you say? basically it is the "walls" or "base" or "foundation" for your code and instructs what your code should do.

2. Name two big differences between ```display: block;``` and ```display: inline;```. the major difference is that display: inline-block allows to set a width and height on the element. Also, with display: inline-block , the top and bottom margins/paddings are respected, but with display: inline they are not.
