...
Semantic HTML introduces meaning to a web page rather than just presentation.

2. Name two big differences between ```display: block;``` and ```display: inline;```.
Block element are on their own line, while inline edlements line up next to each other (like text), so they are on separate line from block elements. An inline element does not cause a line break and does not take up the full width of a page, only the space bounded by its opening and closing tag.
